...
The gate loads persisted direct user messages from the topmost session, including messages outside the model’s rolling/compacted context. Child user-role delegation prompts cannot widen this authority. Assistant prose, tool results, ignored and synthetic messages are excluded; recent child tool calls remain evidence only. Missing/cyclic ancestry, missing root user context, more than 32,000 direct-user characters or an unprojectable/oversized pending action fails closed to manual review for non-safe actions. Deleted root messages cannot be recovered. New instructions sent only to a child session cannot widen the root authorization; update the root conversation explicitly.
