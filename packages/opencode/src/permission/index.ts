...
function forced(metadata: Record<string, unknown> | undefined) {
  return (
...
function covered(entry: PendingEntry, approved: Ruleset, local: Ruleset) {
  ...
  if (forced(entry.info.metadata)) return false // kilocode_change - sensitive requests need an explicit reply
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const reply = Effect.fn("Permission.reply")(function* (input: PermissionV1.ReplyInput) {
      ...
      // kilocode_change start - sensitive requests must be answered by a human; ignore machine approvals
