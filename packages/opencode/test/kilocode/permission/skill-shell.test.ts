...
it.instance(
  "security review - denies prompt injection despite an allow rule",
  () =>
    Effect.gen(function* () {
      const err = yield* fail(
        ask({
          sessionID: SessionID.make("session_security"),
          permission: "security_prompt_injection",
          patterns: ["README.md"],
          metadata: { securityDeny: true, securityReview: true },
          always: [],
          ruleset: [{ permission: "security_prompt_injection", pattern: "*", action: "allow" }],
        }),
      )
